...
export function LeftSidebar() {
  ...
  const navItems = [
    ...
    { label: 'Counter', icon: Activity, href: '/counter' },
    { label: 'My Recap', icon: BarChart3, href: '/recap' },
    { label: 'Prompts', icon: Lightbulb, href: '/prompts' },
    { label: 'Open Stats', icon: Globe, href: '/open' },
    { label: 'Prometheus List', icon: Zap, href: '/token-rich' },
  ...
  return (
    <aside className="hidden lg:flex flex-col w-[220px] shrink-0 border-r border-border p-4 gap-1 h-full overflow-y-auto overscroll-contain sidebar-scroll">
      ...
      <nav className="flex flex-col gap-0.5 flex-1 min-h-0 overflow-y-auto overscroll-contain sidebar-scroll">
        {navItems.map((item) => {
      ...
      <div className="mt-4 pt-4 border-t border-border shrink-0 px-2 space-y-3">
        <div className="flex flex-col gap-1.5">
          <p className="text-[10px] font-semibold tracking-wider uppercase text-muted-foreground/60">Shipped by</p>
